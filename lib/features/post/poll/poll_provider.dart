...
// Get poll for a post
final pollProvider =
    FutureProvider.family<PollModel?, String>((ref, postId) async {
  final response = await SupabaseService.from(SupabaseConstants.pollsTable)
      .select()
      .eq('post_id', postId)
      .maybeSingle();
...
class PollActions {
  ...
  Future<void> vote({
    ...
    final poll = await SupabaseService.from(SupabaseConstants.pollsTable)
        .select()
        .eq('id', pollId)
        .single();
    ...
    await SupabaseService.from(SupabaseConstants.pollsTable)
